...
*/
const initialLength = charactersNames.length;

for (let i = 0; i < femaleCharacters.length; i++) {
  const nameToRemove = femaleCharacters[i].name;
  const index = charactersNames.indexOf(nameToRemove);
  if (index !== -1) {
    charactersNames.splice(index, 1);
  }
} console.log(`Length before: ${initialLength}, Length after: ${charactersNames.length}`);
...
console.log(`You selected ${character.name}, a character who is ${character.height} cm tall and weighs ${character.mass} kg.`);
console.log(`They have ${character.hair_color} hair, ${character.skin_color} skin, and ${character.eye_color} eyes.`);
console.log(`Born in the year ${character.birth_year}, their gender is ${character.gender === 'robot' ? 'robotic' : character.gender}.`);